...
async function handler(request: NextRequest) {
  ...
  if (!userId) {
    ...
    if (!recipientEmail || !subject || !pdfUrl || !fileName) {
      return NextResponse.json({ error: 'Missing required email queue fields' }, { status: 400 });
    }
    ...
    const emailLog = await supabaseAdmin
      .from('email_logs')
      .insert([
        {
          vendor_id: userId,
    ...
    return NextResponse.json({ queued: true, queue: item });
}

export const POST = withApiErrorHandler(handler, '/api/email/queue');
